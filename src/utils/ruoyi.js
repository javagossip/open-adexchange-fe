...
/**
 * 直接下载 Blob 数据为文件
 * @param {Blob} data Blob 文件数据
 * @param {string} fileName 下载的文件名
 * @param {string} mimeType 文件的 MIME 类型
 */
export function downloadBlob(data, fileName, mimeType = 'application/octet-stream') {
  if (!(data instanceof Blob)) {
    console.error('downloadBlob: data is not a Blob object.', data);
    // 如果 data 不是 Blob，尝试将它包装成 Blob，以防万一（虽然理想情况应该是 Blob）
    try {
      data = new Blob([data], { type: mimeType });
    } catch (e) {
      console.error('Failed to convert data to Blob:', e);
      return;
    }
  }
  ...
  window.URL.revokeObjectURL(url); // 释放URL对象，避免内存泄露
}
// 日期格式化
